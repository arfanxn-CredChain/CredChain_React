...
- **2026-06-10 — Force SplitLayout to exactly 100dvh with no scrollbars.** Changed `SplitLayout` from `min-h-screen` to `h-dvh overflow-hidden` on both outer container and right panel, preventing any content from exceeding the viewport. Mobile brand band now capped at `max-h-[40dvh] flex-shrink-0 overflow-hidden`. Content area uses `min-h-0 overflow-hidden` (critical `min-h-0` allows flex children to shrink below natural height). Added `className` prop to `AttestationStamp` component so callers can override sizing; mobile brand band now renders `AttestationStamp` with `className="max-w-[min(160px,18vh)]"` to scale it down on short screens instead of hiding it entirely. `Landing` and `Login` content refactored to use viewport-relative units: `py-[2dvh]` instead of fixed `py-10`/`py-8`, `space-y-[1.5dvh]` instead of `space-y-6`, card padding reduced to `p-6 sm:p-8`. Removed forced `min-h-[2.1lh]`, `min-h-[5lh]`, `min-h-[2lh]` from headings and subtitles — these min-heights prevented content from shrinking to fit short viewports. Added `SplitLayout.test.tsx` (5 tests) asserting `h-dvh`, `overflow-hidden`, `min-h-0`, `max-h-[40dvh]`, and `flex-shrink-0` classes. No new i18n keys.
