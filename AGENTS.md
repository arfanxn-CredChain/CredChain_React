...
8. Add nav entry to `DashboardSidebar.tsx` if user-facing. Filter by role using `NAV_ITEMS.minRole`.
9. Add Vitest tests:
...
**Current count:** **282 unit/component tests across 31 spec files** under `src/`, plus **20 Playwright tests across 3 e2e specs** (`auth.spec.ts`, `public.spec.ts`, `a11y.spec.ts`).
...
- **2026-06-09 — Unify PublicLayout chrome with DashboardLayout.** Public header height now `min-h-[64px]` matching `NavbarDashboard` (was fixed `h-16`, 8px shorter on desktop). Public `<main>` padding now `px-4 pt-4 pb-12 sm:px-8` matching `DashboardLayout` (was `px-4 sm:px-6 lg:px-8 py-6`). Effect: unauthed Help / About / VerifyCredential now render with identical navbar height and content offset to their authed (`DashboardLayout`) counterparts. `SplitLayout` (Landing, Login) intentionally unchanged — it uses its own centered-content pattern. Documented BackLink behavior + placement in DESIGN_SYSTEM.md §6.2.1.
- **2026-06-09 — Reset scroll on navigation.** Added `useScrollToTop()` hook (`src/shared/hooks/useScrollToTop.ts`) that resets both the window scroller and the `#main` overflow container to `(0, 0)` whenever the pathname changes. Wired into `DashboardLayout`, `PublicLayout`, and `SplitLayout` so every page transition starts at the top. React Router v7's built-in `ScrollRestoration` only manages the document scroller; `DashboardLayout`'s `main#main` uses `overflow-y-scroll` so the document never scrolls — both containers must be reset.
...
- **2026-06-09 — Navbar + dropdown polish.** Fixed NavbarDashboard vertical centering (separated `safe-area-top` from flex row's `py-*` to avoid CSS utilities-layer cascade override). DashboardLayout `main` now uses `overflow-y-scroll [scrollbar-gutter:stable]` so route-switches with different content heights no longer reflow the navbar. Profile dropdown set to `modal={false}` to avoid scroll-lock layout shifts; trigger button uses `focus-visible:[outline:none]` to defeat the global `:focus-visible` gold outline; focus indicator moved onto the avatar itself via `group-focus-visible:ring-gold` (clean circular ring, keyboard-only). Avatar gets adaptive ring (`ring-surface` on mobile navy header, `ring-gray-200` on desktop light header). Removed the gold `<DecorBlob>` from the mobile navy band in `SplitLayout` (was clipped by `overflow-hidden` and the safe-area gap on notched iOS). Tinted all dropdown shadows `shadow-navy/20` (was `shadow-gray-200/50` which appeared as a white smudge on dark backgrounds).
