...
- **2026-06-11 — Re-add `max-w-7xl` to NavbarDashboard inner row (background stays full-width).** Re-adding `mx-auto max-w-7xl` to the NavbarDashboard flex row — the `<header>` background still spans full width but the menu/search/avatar row is now centered at `max-w-7xl` and aligns with the list pages below. The previous "full width" revert was correct for the _content area_ (DashboardLayout `<main>`) but not for the _navbar_ — navbar items need a container boundary so ultra-wide screens don't push the avatar cluster 2000px away from the menu button. This matches the same pattern as NavbarPublic: full-width background, centered content. No test changes (no behavioral change beyond visual containment).
